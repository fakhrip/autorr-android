/* octave.js (C) 2022-present fakhrip [https://github.com/fakhrip] */
var octavejs;
(function (factory) {
  ...
  octavejs.version = "1.0.0";

  var evalForEach = function evalForEach (arr, op, operand) {
    var res = [];
  ...
  var isscalar = function isscalar (x) {
    return typeof x == "number";
  };

  var isreal = function isreal (x) {
  ...
  var isInclude = function isInclude (arr, val) {
    for (var i = 0; i < arr.length; i++) {
      if (arr[i] == val) {
        return true;
      }
  ...
  var zeros = function zeros (target_dim) {
    return genArr(target_dim, 0);
  };

  var ones = function ones (target_dim) {
    return genArr(target_dim, 1);
  };

  var genArr = function genArr (dimensions, value) {
  ...
  var getElement = function getElement (array, indices) {
    if (indices.length == 0) {
      return array;
    } else {
  ...
  // https://stackoverflow.com/questions/12588618/javascript-n-dimensional-array-creation
  var doGenArr = function doGenArr (dimensions, value) {
  ...
  var find = function find (arr, fun, max_n) {
    var res = [];
  ...
  var find2arr = function find2arr (arr1, arr2, fun, max_n) {
    var res = [];
  ...
  var cat = function cat (dim, arr1, arr2) {
  ...
  var doCat = function doCat (dim, arr1, arr2) {
    var res = [];
    return res;
  };
  ...
  var cumsum = function cumsum (arr) {
  ...
  var toColVec = function toColVec (arr) {
    var res = [];
  ...
  var toRowVec = function toRowVec (arr) {
    if (size(arr)[0] == 1) {
      return arr;
    }
  ...
  var sortWithIndeces = function sortWithIndeces (toSort, isAscending) {
  ...
  var bandpower = function bandpower (ps, mode) {
    if (mode == undefined) {
      var mode = "psd";
    }
  ...
  var getIndicesAroundPeak = function getIndicesAroundPeak (arr, peakIndex, searchWidth) {
  ...
  var freqToBin = function freqToBin (fAxis, Freq) {
    return argmin(math.abs(evalForEach(fAxis, "-", Freq)));
  };

  var getPeakInArea = function getPeakInArea (psd, faxis, estimation, searchWidthHz) {
    /*
  ...
  var getHarmonics = function getHarmonics (fund, sr, nHarmonics, aliased) {
  ...
  var nextpow2 = function nextpow2 (num) {
    return Math.ceil(Math.log2(Math.abs(num)));
  };

  var sumsq = function sumsq (arr) {
    var sum = 0;
    for (let i = 0; i < arr.length; i++) {
  ...
  // ===
  var periodogram = function periodogram (x, window, nfft, fs, range) {
  ...
  // Ported based one
  // https://www.mathworks.com/matlabcentral/fileexchange/36423-ifft-using-fft?s_tid=srchtitle
  var ifft = function ifft (input) {
    var conj = math.conj(input);
    var transpose = math.transpose(conj);
    var fftRes = fft(transpose);
    var temp = math.conj(fftRes);
    return math.divide(temp, input.length);
  };

  // Ported based on
  // https://www.mathworks.com/matlabcentral/fileexchange/13606-fast-fourier-transform-algorithm
  var fft = function fft (input) {
    var ss = input.length;
  ...
  // /usr/share/octave/5.2.0/m/signal/periodogram.m
  // ===
  var kaiser = function kaiser (m, beta) {
    if (beta == undefined) {
      var beta = 0.5;
    }
  ...
  // ===
  var postpad = function postpad (x, l, c, dim) {
    var nd = ndims(x);
    var sz = size(x);
  ...
  // ===
  var butter = function butter (n, wc, type, nout) {
  ...
  // ===
  var filter = function filter (b, a, x, w) {
    var N = a.length;
    var M = b.length;
    var L = x.length;
  ...
  // ===
  var filtfilt = function filtfilt (b, a, x) {
  ...
  // ===
  var bilinear = function bilinear (sz, sp, sg, T) {
    var p = sp.length;
    var z = sz.length;
    ...
    var sz_prod = 1;
    if (new_sz.length != 0) {
      sz_prod = math.prod(new_sz);
    }
    var zg = math.re((sg * sz_prod) / math.prod(new_sp));
  ...
  // ===
  var sftrans = function sftrans (sz, sp, sg, w, stop) {
    const C = 1;
    var p = sp.length;
    var z = sz.length;
  ...
  // ===
  var detrend = function detrend (x, p) {
    if (p == undefined) {
      var p = 1;
    }
  ...
  // ===
  var findpeaks = function findpeaks (data, minH, minD, minW, maxW) {
  ...
  // ~/octave/signal-1.4.1/findpeaks.m
  // ===
  // Note: currently not a full implementation
  var hilbert = function hilbert (f, N, dim) {
    N = N == undefined ? [] : N;
    ...
    if (isempty(dim)) {
      ...
      for (let i = 0; i < arrSize.length; i++) {
        if (arrSize[i] > 1) total += arrSize[i];
      }
      ...
      if (dim.length != 1) {
        throw "HILBERT: dim must be a scalar.";
      }
      if (dim % 1 != 0) {
        throw "HILBERT: dim must be an integer.";
      }
      if (dim < 1 || dim > D) {
        throw "HILBERT: dim must be in the range from 1 to " + D + ".";
      }
    }

    if (N.length > 1) {
      throw "N must be a scalar.";
    } else if (!isempty(N) && N % 1 != 0) {
    ...
    var permutedsize = size(f);
    permutedsize[0] = N;

    // Reshape f to a matrix.
    f = math.reshape(f, [size(f)[0], f.length / size(f)[0]]);
    var W = size(f)[1];
    ...
    if (N > 2) {
      ...
      if (N % 2 == 0) {
        ...
        temp.push(f[0]);
        for (let i = 1; i < N / 2; i++) {
          temp.push(math.multiply(f[i], 2));
        }
        temp.push(f[N / 2 + 1]);
        temp.push.apply(temp, math.flatten(zeros([N / 2 - 1, W])));
        ...
        temp.push.apply(temp, math.flatten(zeros([(N - 1) / 2, W])));
    ...
    permutedsize[0] = f.length;
    f = math.reshape(f, permutedsize);

    return f;
  };

  var pprint = function pprint (arr) {
    print(JSON.stringify(arr, null, 2));
  };

  octavejs.evalForEach = evalForEach;
  octavejs.isempty = isempty;
  octavejs.isvector = isvector;
  octavejs.isscalar = isscalar;
  octavejs.isreal = isreal;
  octavejs.isInclude = isInclude;
  octavejs.pinv = pinv;
  octavejs.growArr = growArr;
  octavejs.zeros = zeros;
  octavejs.ones = ones;
  octavejs.genArr = genArr;
  octavejs.getElement = getElement;
  octavejs.doGenArr = doGenArr;
  octavejs.size = size;
  octavejs.ndims = ndims;
  octavejs.find = find;
  octavejs.find2arr = find2arr;
  octavejs.rot90 = rot90;
  octavejs.cat = cat;
  octavejs.doCat = doCat;
  octavejs.arraysEqual = arraysEqual;
  octavejs.cumsum = cumsum;
  octavejs.toColVec = toColVec;
  octavejs.toRowVec = toRowVec;
  octavejs.sortWithIndeces = sortWithIndeces;
  octavejs.unique = unique;
  octavejs.argmax = argmax;
  octavejs.argmin = argmin;
  octavejs.snr = snr;
  octavejs.bandpower = bandpower;
  octavejs.getIndicesAroundPeak = getIndicesAroundPeak;
  octavejs.freqToBin = freqToBin;
  octavejs.getPeakInArea = getPeakInArea;
  octavejs.getHarmonics = getHarmonics;
  octavejs.nextpow2 = nextpow2;
  octavejs.sumsq = sumsq;
  octavejs.periodogram = periodogram;
  octavejs.ifft = ifft;
  octavejs.fft = fft;
  octavejs.kaiser = kaiser;
  octavejs.poly = poly;
  octavejs.postpad = postpad;
  octavejs.butter = butter;
  octavejs.fliplr = fliplr;
  octavejs.flipud = flipud;
  octavejs.flip = flip;
  octavejs.filter = filter;
  octavejs.filtfilt = filtfilt;
  octavejs.bilinear = bilinear;
  octavejs.sftrans = sftrans;
  octavejs.detrend = detrend;
  octavejs.findpeaks = findpeaks;
  octavejs.hilbert = hilbert;
  octavejs.pprint = pprint;
});
